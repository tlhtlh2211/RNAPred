...
import pandas as pd
import math
from rnapred.utils import postprocessing, mat2bp
from rnapred.metrics import contact_f1
...
class RNAPrediction(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.device = device
        self.class_weight = torch.tensor([negative_weight, 1.0]).float().to(device)
    ...
    def forward(self, batch):
        ...
        if self.interaction_prior != "none":
            ...
            x1 = torch.zeros([batch_size, 2, L, L]).to(self.device)
            x1[:, 0, :, :] = y0
            x1[:, 1, :, :] = prob_mat
        else:
            x1 = y0.unsqueeze(1)
